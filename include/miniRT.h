...
t_shape			*add_sphere_bonus(
					t_rt *rt, char **coordinates, char **colors, double dia);
...
t_file			validate_args(
					char **element, int coord_i, int normal_i, int color_i);
...
t_tuple			pattern_at_object(t_pattern pattern,
					t_shape shape, t_tuple world_point);
t_tuple			shade_hit(t_scene *world, t_intersection comps, int remaining);
t_tuple			lighting(t_intersection comps,
					t_shape shape, t_light light, int in_shadow);
